...
export const metadata = { title: "Favorites · SpectraForge" };

export default async function FavoritesPage() {
  const supabase = await createClient();
  const [{ data }, machines, materials] = await Promise.all([
    supabase.from("recipes").select("*, machines(name, type)").eq("favorite", true).order("created_at", { ascending: false }),
    getMachineOptions(),
    getMaterialOptions(),
  ]);
  ...
  return <RecipesScreen recipes={recipes} machines={machines} materials={materials} defaultMachineId={machines[0]?.id ?? null} favoritesOnly />;
}
